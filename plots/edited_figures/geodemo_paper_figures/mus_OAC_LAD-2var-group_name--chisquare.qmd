...
governance_chisq_expected_1_tbl <-
  governance_chisq %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
governance_chisq_expected_5_tbl <-
  governance_chisq %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
Only `r governance_chisq_expected_1_tbl %>% filter(one_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 1 and only `r governance_chisq_expected_5_tbl %>% filter(five_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 5.
...
governance_chisq_simpv_expected_1_tbl <-
  governance_chisq_simpv %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
governance_chisq_simpv_expected_5_tbl <-
  governance_chisq_simpv %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
As above, only `r governance_chisq_simpv_expected_1_tbl %>% filter(one_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 1 and only `r governance_chisq_simpv_expected_5_tbl %>% filter(five_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 5.
...
$$
\chi^2(`r governance_chisq %$% parameter %>% extract("df")`,`r governance_chisq %$% observed %>% sum()`) = `r governance_chisq %$% statistic %>% round(digits = 2)` \text{, } `r if (governance_chisq %$% p.value < 0.001) "p < .001" else if (governance_chisq %$% p.value < 0.01) "p < .01" else paste("p =", governance_chisq %$% p.value %>% round(digits = 2))`
$$
...
$$
\chi^2(,`r governance_chisq_simpv %$% observed %>% sum()`) = `r governance_chisq_simpv %$% statistic %>% round(digits = 2)` \text{, } `r if (governance_chisq_simpv %$% p.value < 0.001) "p < .001" else if (governance_chisq_simpv %$% p.value < 0.01) "p < .01" else paste("p =", governance_chisq_simpv %$% p.value %>% round(digits = 2))`
$$
...
subject_matter_chisq_expected_1_tbl <-
  subject_matter_chisq %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
subject_matter_chisq_expected_5_tbl <-
  subject_matter_chisq %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
All cell have expected values greater or equal to 1. Only `r subject_matter_chisq_expected_5_tbl %>% filter(five_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 5.
...
subject_matter_chisq_simpv_expected_1_tbl <-
  subject_matter_chisq_simpv %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
subject_matter_chisq_simpv_expected_5_tbl <-
  subject_matter_chisq_simpv %$%
  expected %>% 
  as_tibble() %>% 
  pivot_longer(
...
All cell have expected values greater or equal to 1. Only `r subject_matter_chisq_simpv_expected_5_tbl %>% filter(five_or_greater) %>% pull(perc_of_cells) %>% round(digits = 2)`% of cells have expected values greater or equal to 5.
...
$$
\chi^2(`r subject_matter_chisq %$% parameter %>% extract("df")`,`r subject_matter_chisq %$% observed %>% sum()`) = `r subject_matter_chisq %$% statistic %>% round(digits = 2)` \text{, } `r if (subject_matter_chisq %$% p.value < 0.001) "p < .001" else if (subject_matter_chisq %$% p.value < 0.01) "p < .01" else paste("p =", subject_matter_chisq %$% p.value %>% round(digits = 2))`
$$

However, `R` returned the warning `Chi-squared approximation may be incorrect`, due to the very sparse nature of the table. Thus, a second test was run with simulated p-value (`based on 2000 replicates`). The results are the same, as reported below.

$$
\chi^2(,`r subject_matter_chisq_simpv %$% observed %>% sum()`) = `r subject_matter_chisq_simpv %$% statistic %>% round(digits = 2)` \text{, } `r if (subject_matter_chisq_simpv %$% p.value < 0.001) "p < .001" else if (subject_matter_chisq_simpv %$% p.value < 0.01) "p < .01" else paste("p =", subject_matter_chisq_simpv %$% p.value %>% round(digits = 2))`
$$
