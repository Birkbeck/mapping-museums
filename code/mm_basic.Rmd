...
print(get_open_museums_by_year(2019, museums_bg@data, 'all', 'all'))
...
museums2019sdf = FUN_get_museums_open_in_a_year(museums_bg, 2019)
stopifnot(nrow(museums2019sdf)==3364)
...
FUN_dfsummary_to_file(museums2019sdf, paste0(basicfold,"mus2019_summary_stats.txt"))
...
for (var in c("subject_matter","subject_matter_simpl","subject_matter_simpl_aggr",
  ...
  # 2019 MUSEUMS
  x = format_data_for_summary( museums2019sdf@data[,var] )
  df = p_summary_list( x, paste0('mus2019-',var) )
  df = subset(df,df$VAL.Freq > 0)
  write_xlsx(df, paste0(basicfold, 'mus2019_summary_var-',var,".xlsx") )
  rm(df,x)
}

names(museums2019sdf)
...
for(var1 in c("size","subject_matter","governance_simpl","subject_matter_simpl","accreditation","country_name")){
  for(var2 in c("governance","governance_simpl","accreditation","subject_matter","subject_matter_simpl","region_name")){
    ...
    if (var1!=var2) FUN_twoVarStats(museums2019sdf@data, var1, var2, '../plots/basic_stats/two_var_stats/mus2019_')
    if (var1!=var2) FUN_twoVarStats(museums2019sdf@data, var2, var1, '../plots/basic_stats/two_var_stats/mus2019_')
  }
}
...
# the summaries? So, I'm looking at mus2019_summary_var-classification_2018-just_accredited.xlsx
# and the equivalent for total 2019 museums. What's surprising is that there isn't a huge amount of difference 
...
# museums to all2019 museums, large houses jumps +2%, transport +1% and so on? that would make it much 
# easier for me to see where change happens.
# Also, would you generate a summary table for unaccredited museums please?

# 2019 MUSEUMS class 18 stats
for (var in c("subject_matter")){
  df = as.data.frame.matrix(table(museums2019sdf@data[,c(var,"accreditation")]))
  #print(nrow(mus))
  df$V1=NULL
  df$all2019 = rowSums(df)
  print(sum(df$all2019))
  df$all2019_PC = round(df$all2019/nrow(museums2019sdf)*100,2)
  df$Accredited_PC = round(df$Accredited/nrow(museums2019sdf)*100,2)
  df$Unaccredited_PC = round(df$Unaccredited/nrow(museums2019sdf)*100,2)
  ...
  `*` = row.names(df)
  df = cbind( `*`, df )
  fn = paste0('../plots/basic_stats/two_var_stats/mus2019_summary_2var_extended-',var,"-vs-accreditation.xlsx")
  write_xlsx(df, fn)
  rm(df,var)
}
...
if (FALSE){
  for(var1 in c("size","aim_size_designation","accreditation")){
    for(var2 in c("ace_size_designation","aim_size_designation")){
      ...
      if (var1!=var2) FUN_twoVarStats(museums2019sdf, var1, var2, '../plots/basic_stats/two_var_stats/mus2019_')
    }}
}

rm(var1,var2,basicfold,museums2019sdf)
rm(alldf)
...
stopifnot(nrow(museums2019sdf)>0)

gen_stats_country_region(museums_bg@data,'all')
gen_stats_country_region(museums2019sdf@data,'22')
...
write_xlsx(df, '../plots/openclose/openclose_cumulative/years_openclose_stats_pc_change_report2019.xlsx')
rm(df)
```
...
generate_1var_from_cumul_years_ = function(cumul_years, lab){
  ...
  # compare 1960 with 2019
  df = subset( cumul_years, cumul_years$year==1960 | cumul_years$year==2019)
  unique(df$var_name)
  print(names(df))
  #BROKEN CODE HERE
  ...
  names(dfopen) = c("var_name","var_value","open1960","open2019" )
  names(dfclosed) = c("var_name","var_value","close1960","close2019" )
  #View(dfopen)
  #View(dfclosed)
  ...
  dfopen$close2019 = dfclosed$close2019
  dfopen$rowid=NULL
  rm(dfclosed)
  
  #View(dfopen)
  dfopen$geo_label = replace_gss_codes_with_NAs(dfopen$var_value)
  dfopen = dfopen[,c("var_name","var_value","geo_label","open1960","open2019",'close1960','close2019')]
  dfopen$diffN = dfopen$open2019 - dfopen$open1960
  dfopen$growth = ifelse( dfopen$open1960 >= 1, 
        round((dfopen$open2019 - dfopen$open1960) / dfopen$open1960 * 100,1), 
        NA )
  dfopen$open2019 = round(dfopen$open2019,1)
  dfopen$close2019 = round(dfopen$close2019,1)
  dfopen$open1960 = round(dfopen$open1960,1)
  dfopen$close1960 = round(dfopen$close1960,1)
  dfopen$diffN = round(dfopen$diffN,1)
  dfopen$closedpc = ifelse(dfopen$open2019>=1,
        round( dfopen$close2019 / (dfopen$open2019+dfopen$close2019) * 100, 1 ),
        NA)
  return(dfopen)
}
...
calc_growth_by_two_vars_ = function( musdf, var1, var2 ){
  ...
  for (val1 in as.character(unique(musdf[,var1]))){
    ...
    for (val2 in as.character(unique(musdf[,var2]))){
      ...
      row$open2019 = row2$cumul_open_wei_N
      row$close2019 = row2$cumul_closed_wei_N
      #print(names(row))
      stopifnot(ncol(row)==9)
      names(row) = c("year","var2","val2","open1960","close1960",
        "var1","val1","open2019","close2019")
      # calc growth
      row$diffN = row$open2019 - row$open1960
      row$growth = ifelse( row$open1960 >= 1, 
                        round((row$open2019 - row$open1960) / row$open1960 * 100, 1), 
                        NA )
      row$open1960 = round(row$open1960,1)
      row$open2019 = round(row$open2019,1)
      row$diffN = round(row$diffN,1)
      row$closedpc = ifelse(row$open2019>=1,
                        round( row$close2019 / (row$open2019+row$close2019) * 100, 1 ),
                        NA)
      #View(row)
      # save result row
      #print('debug6')
  ...
  openm_df = openm_df[,c("year","var1","val1",'var1_geo_label',"var2","val2",
                         "open1960","close1960",'open2019','close2019','closedpc',
                         'diffN','growth')]
  return(openm_df)
}
...
gen_growth_maps_ = function(dfopen, growthtwodf, lab, growth_outfold ){
  ...
  # "GSS" "NAME" "var_name" "geo_label" "open1960"  "open2019"  "diffN" "growth"
  ...
  VARS_GROWTH = c('growth','diffN','open1960','open2019','close2019','closedpc')
...
plot_choro_map_museum_growth_by_var_ = function( lab, growthtwodf, geo_sdf, geovar, var_name, growth_outfold, engl_regs ){
  ...
  for (val in unique(subgrowthtwodf$val2)){
    ...
    for(f in c('growth','diffN','open2019','close2019')){  # ,'open1960','')){
      for(q in c('jenks')){ # 'quantile','equal',{
...
generate_all_growth_stats = function(cumul_years, lab, mdf){
  ...
  write_xlsx(dfopen, paste0(growth_outfold,'openclose_1960_2019_pc_change-1vars-',lab,'.xlsx'))
  
  foutn = paste0(growth_outfold,'openclose_1960_2019_pc_change-2vars-',lab,'.xlsx')
  if (T){
    # gen 2 vars stats
...
### Custom figures for publications

Figures for overview paper 2022:
