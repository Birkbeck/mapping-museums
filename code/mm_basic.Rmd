...
load_uk_dataset <- function( name ){
  ds = readRDS(paste0("../datasets/uk_datasets/",
                      name))
  stopifnot( nrow(ds)>0 )
...
calc_growth_by_two_vars_ = function( musdf, var1, var2 ){
  View(musdf)
  print('calc_growth_by_two_vars_')
  years = c(1960,2022)
  openm_df = data.frame()
...
generate2varsCombinations_ = function( indf ){
  ...
  for(var2 in c('subject_matter_simpl','size','governance','governance_simpl','accreditation')){
    if (var1!=var2){
      ...
      print('debug5')
      growthtwodf = smartbind( growthtwodf, df )
      rm(df)
  }}}
  #View(growthtwodf)
  return(growthtwodf)
...
generate_all_growth_stats = function(cumul_years, lab, mdf){
  ...
  if (T){
    ...
    write_xlsx(growthtwo_df,
               paste0(growth_outfold,'openclose_1960_2022_pc_change-2vars-',lab,'.xlsx'))
...
cumul_years3 = read_excel('../plots/openclose/openclose_cumulative/years_openclose_stats_cumulative-mus_all-full.xlsx')
cumul_years_indep3 = read_excel('../plots/openclose/openclose_cumulative/years_openclose_stats_cumulative-mus_indep-full.xlsx')
cumul_years_lauth3 = read_excel('../plots/openclose/openclose_cumulative/years_openclose_stats_cumulative-mus_lauth-full.xlsx')
