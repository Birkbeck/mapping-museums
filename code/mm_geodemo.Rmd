...
gss_lib = load_uk_dataset("gss_lib.rds")
...
#  - National
#  - Medium and large local authority museums
museums2017uksdf$special_type = as.factor(apply( museums2017uksdf@data, 1, function(row){
  #print(row['size'])
  if (row['size'] == "small" & row['governance_simpl'] == "independent") return("sm_indep")
  if (row['governance'] == "/Government/National") return("national")
  if (row['size'] %in% c("medium","large") & row['governance']=="/Government/Local Authority") return("local_auth")
  return(NA)
}))
